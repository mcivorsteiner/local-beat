module Songkick
  ...
  def extract_event_data(events)
    events_data = events.map do |event|
      ...
      { sk_event_id: sk_event_id, display_name: display_name, date: date, artists: artists, headliner: headliner, headliner_id: headliner_id.first, metro_area: metro_area, state: state, venue_name: venue_name, popularity: popularity, location: location, event_type: event_type, uri: uri, time: time }
    end
